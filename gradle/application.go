...
var (
	DefaultArguments = []string{"--no-daemon", "-x", "test", "build"}
	DefaultTarget    = filepath.Join("build", "libs", "*.[jw]ar")
)
...
func (a Application) Contribute(layer libcnb.Layer) (libcnb.Layer, error) {
	a.Logger.Body(bard.FormatUserConfig("BP_GRADLE_BUILD_ARGUMENTS", "the arguments passed to the build system",
		strings.Join(DefaultArguments, " ")))
	a.Logger.Body(bard.FormatUserConfig("BP_GRADLE_BUILT_MODULE", "the module to find application artifact in", "<ROOT>"))
	a.Logger.Body(bard.FormatUserConfig("BP_GRADLE_BUILT_ARTIFACT", "the built application artifact", DefaultTarget))
...
func (a Application) ResolveArguments() ([]string, error) {
	...
	if s, ok := os.LookupEnv("BP_GRADLE_BUILD_ARGUMENTS"); ok {
...
func (a Application) ResolveArtifact() (string, error) {
	...
	if s, ok := os.LookupEnv("BP_GRADLE_BUILT_MODULE"); ok {
		pattern = filepath.Join(s, pattern)
	}
	if s, ok := os.LookupEnv("BP_GRADLE_BUILT_ARTIFACT"); ok {
		pattern = s
	}
